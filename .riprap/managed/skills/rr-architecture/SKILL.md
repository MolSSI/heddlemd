...
description: >-
  ...
  any `/rr-plan` or `/rr-implement` work.
  ...
  SKIP: the user asks to plan or specify a single feature (use rr-plan instead); the user asks to
  implement or change code (use rr-implement instead).
...
many future features. Everything finer-grained than that is deferred to `/rr-plan`.
...
that `/rr-plan` produces: those describe individual features in testable detail; this describes the
system they all live in.

Capture only what has architectural leverage. It is **not** important to nail down every detail here
(that is what `/rr-plan` is for). The goal is to glean the information that would tend to heavily
...
detail of one feature, leave it for `/rr-plan`.

## Seed Context

This project is named **HeddleMD** and is written in **rust**. At creation it was
described as:
...
feature's specification into a `/rr-plan` requirements file instead.
...
in existing `/rr-plan` requirements files, point that out to the user rather than silently editing
those files — reconciling them is a separate `/rr-plan` task.

## Traceability

`rqm/ARCHITECTURE.md` is a high-level orientation document, not a requirements specification. Unlike
the feature files produced by `/rr-plan`, it is **exempt from the `rq-XXXXXXXX` traceability ID
