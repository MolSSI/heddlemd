...
fn cell_counts_floor_of_l_over_search_radius() {
    ...
    let nl = NeighborListState::new(device, box_n(10.0), 0, 1.0, 8, 0.3).unwrap();
    assert_eq!(nl.n_cells, [7, 7, 7]);
    let expected = 10.0_f32 / 7.0;
    for a in 0..3 {
        assert!(
            (nl.cell_size[a] - expected).abs() < 1.0e-6,
            "cell_size[{}] = {}, expected {}",
            a,
            nl.cell_size[a],
            expected
...
fn reject_box_admitting_fewer_than_three_cells() {
    let device = init_device().unwrap();
    let result = NeighborListState::new(device, box_n(10.0), 0, 1.0, 8, 3.0);
    match result {
...
fn particle_count_zero_builds_and_runs() {
    let device = init_device().unwrap();
    let mut nl = NeighborListState::new(device.clone(), box_n(10.0), 0, 1.0, 8, 0.3).unwrap();
...
fn single_particle_yields_empty_neighbor_list() {
    let device = init_device().unwrap();
    let mut nl = NeighborListState::new(device.clone(), box_n(10.0), 1, 1.0, 8, 0.3).unwrap();
...
fn neighbor_list_contains_all_within_search_radius_and_is_sorted() {
    ...
    let mut nl =
        NeighborListState::new(device.clone(), box_n(10.0), 4, r_cut, max_neighbors, r_skin)
            .unwrap();
    let mut timings = Timings::new(device.clone()).unwrap();
    nl.rebuild(&buffers, &mut timings).unwrap();
...
fn neighbor_list_uses_minimum_image() {
    ...
    let buffers = ParticleBuffers::new(device.clone(), &state).unwrap();
    let mut nl = NeighborListState::new(device.clone(), box_n(10.0), 2, 0.7, 8, 0.3).unwrap();
    let mut timings = Timings::new(device.clone()).unwrap();
    nl.rebuild(&buffers, &mut timings).unwrap();
    let counts = device.dtoh_sync_copy(&nl.neighbor_counts).unwrap();
    let list = device.dtoh_sync_copy(&nl.neighbor_list).unwrap();
...
fn build_signals_overflow() {
    ...
    let buffers = ParticleBuffers::new(device.clone(), &state).unwrap();
    let mut nl = NeighborListState::new(device.clone(), box_n(10.0), 6, 1.0, 2, 0.3).unwrap();
    let mut timings = Timings::new(device).unwrap();
...
fn two_rebuilds_with_identical_positions_agree() {
    ...
    let mut timings = Timings::new(device.clone()).unwrap();
    let mut nl_a =
        NeighborListState::new(device.clone(), box_n(10.0), 6, 1.0, 16, 0.3).unwrap();
    let mut nl_b =
        NeighborListState::new(device.clone(), box_n(10.0), 6, 1.0, 16, 0.3).unwrap();
    ...
    let offsets_a = device.dtoh_sync_copy(&nl_a.cell_offsets).unwrap();
    let offsets_b = device.dtoh_sync_copy(&nl_b.cell_offsets).unwrap();
    let ids_a = device.dtoh_sync_copy(&nl_a.sorted_particle_ids).unwrap();
    let ids_b = device.dtoh_sync_copy(&nl_b.sorted_particle_ids).unwrap();
...
fn displacement_check_zero_immediately_after_rebuild() {
    ...
    let buffers = ParticleBuffers::new(device.clone(), &state).unwrap();
    let mut nl = NeighborListState::new(device.clone(), box_n(10.0), 3, 1.0, 8, 0.3).unwrap();
    let mut timings = Timings::new(device).unwrap();
    nl.rebuild(&buffers, &mut timings).unwrap();
    let max_disp = nl.displacement_check(&buffers, &mut timings).unwrap();
...
fn first_pre_step_unconditionally_rebuilds() {
    ...
    let buffers = ParticleBuffers::new(device.clone(), &state).unwrap();
    let mut nl = NeighborListState::new(device.clone(), box_n(10.0), 2, 1.0, 8, 0.3).unwrap();
    let mut timings = Timings::new(device).unwrap();
    assert!(nl.needs_rebuild);
