...
/// Launch the on-device exclusion-tile build sub-pipeline: invert the
/// sort, count and scatter modified pairs by i-block, sort each i-block's
/// pairs by j-block, emit one tile per distinct block pair with an
/// order-independent OR of the lane bits, and publish the count + CSR +
/// high-water/overflow bits — all on the device, no host round-trip.
///
/// The caller zeros `iblock_pair_count` and `iblock_cursor` before this
/// call. Steps that iterate modified pairs are skipped when `n_modified`
/// is zero; the scans and emit still run and produce an empty tile set.
///
/// rq-dbffee81 rq-8945395f rq-5f871bd5 rq-fa0b3d10 rq-67a09135
#[allow(clippy::too_many_arguments)]
pub fn build_exclusion_tiles_device(
    kernels: &Kernels,
    sorted_particle_ids: &CudaSlice<u32>,
    modified_pairs: &CudaSlice<u32>,
    n_modified: u32,
    n_atoms: u32,
    n_blocks: u32,
    exclusion_tiles_capacity: u32,
    exclusion_tiles_high_water_mark: u32,
    atom_slot: &mut CudaSlice<u32>,
    iblock_pair_count: &mut CudaSlice<u32>,
    iblock_pair_offset: &mut CudaSlice<u32>,
    iblock_cursor: &mut CudaSlice<u32>,
    pair_bj: &mut CudaSlice<u32>,
    pair_lane: &mut CudaSlice<u32>,
    iblock_tile_count: &mut CudaSlice<u32>,
    excl_jblock_offsets: &mut CudaSlice<u32>,
    exclusion_tile_iblocks: &mut CudaSlice<u32>,
    exclusion_tile_jblocks: &mut CudaSlice<u32>,
    exclusion_tile_masks: &mut CudaSlice<u32>,
    excl_jblocks: &mut CudaSlice<u32>,
    interaction_count: &mut CudaSlice<u32>,
    neighbor_status: &mut CudaSlice<u32>,
) -> Result<(), GpuError> {
    if n_blocks == 0 {
    ...
    let single = ::cudarc::driver::LaunchConfig {
        grid_dim: (1, 1, 1),
        block_dim: (1, 1, 1),
        shared_mem_bytes: 0,
    };

    // 1. Invert the sort: atom_slot[sorted_particle_ids[s]] = s. rq-5f871bd5
    if n_atoms > 0 {
        let func = kernels.neighbor.build_atom_slot.clone();
        unsafe {
            func.launch(
                launch_config(n_atoms),
                (sorted_particle_ids, &mut *atom_slot, n_atoms),
            )
            .map_err(GpuError::from)?;
        }
    }

    // 2. Count modified pairs per i-block. rq-8945395f
    if n_modified > 0 {
        let func = kernels.neighbor.count_pairs_by_iblock.clone();
        unsafe {
            func.launch(
                launch_config(n_modified),
                (modified_pairs, n_modified, &*atom_slot, &mut *iblock_pair_count),
            )
            .map_err(GpuError::from)?;
        }
    }

    // 3. Exclusive scan -> per-i-block pair offsets (no clamp: the pair
    //    records buffer is sized to the exact modified-pair count). rq-67a09135
    {
        let func = kernels.neighbor.serial_prefix_scan_u32.clone();
        unsafe {
            func.launch(single, (&*iblock_pair_count, &mut *iblock_pair_offset, n_blocks, 0u32))
                .map_err(GpuError::from)?;
        }
    }

    // 4. Scatter (bj, lane) records grouped by i-block. rq-8945395f
    if n_modified > 0 {
        let func = kernels.neighbor.scatter_pairs_by_iblock.clone();
        unsafe {
            func.launch(
                launch_config(n_modified),
                (
    ...
    // 5. Per i-block: sort by bj, count distinct tiles. rq-8945395f
    {
        let func = kernels.neighbor.sort_and_count_tiles_by_iblock.clone();
        unsafe {
            func.launch(
                launch_config(n_blocks),
                (
                    &*iblock_pair_offset,
                    &mut *pair_bj,
                    &mut *pair_lane,
                    &mut *iblock_tile_count,
                    n_blocks,
                ),
            )
            .map_err(GpuError::from)?;
    ...
    // 6. Exclusive scan -> tile offsets (= excl_jblock_offsets), clamped to
    //    capacity so the CSR find_blocks reads stays in-bounds on overflow.
    //    rq-67a09135
    {
        let func = kernels.neighbor.serial_prefix_scan_u32.clone();
        unsafe {
    ...
    // 7. Emit tiles + masks + CSR jblocks. rq-8945395f rq-fa0b3d10
    {
        let func = kernels.neighbor.emit_tiles_by_iblock.clone();
        unsafe {
            func.launch(
                launch_config(n_blocks),
                (
                    ...
                    &*excl_jblock_offsets,
                    exclusion_tiles_capacity,
                    &mut *exclusion_tile_iblocks,
                    &mut *exclusion_tile_jblocks,
                    &mut *exclusion_tile_masks,
                    &mut *excl_jblocks,
                    n_blocks,
                ),
            )
    ...
    // 8. Publish count[2] and set exclusion high-water/overflow bits. rq-67a09135
    {
        let func = kernels.neighbor.set_exclusion_status.clone();
        unsafe {
            func.launch(
                single,
                (
                    ...
                    n_blocks,
                    exclusion_tiles_capacity,
                    exclusion_tiles_high_water_mark,
                    &mut *interaction_count,
                    &mut *neighbor_status,
                ),
            )
