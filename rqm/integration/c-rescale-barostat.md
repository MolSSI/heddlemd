...
Per timestep, the C-rescale barostat's `apply` runs the following four
device-side steps as its own standalone launches:
...
All four kernels run inside `apply`. The kinetic-energy and virial
reductions are fusion barriers (`docs/architecture.md`), so the
position rescale is a standalone launch rather than a composed fragment.
...
## Position Rescale <!-- rq-61f241f6 -->

The per-particle position rescale is the standalone
`rescale_positions_device_factor` kernel, launched from `apply` as its
step 4. Each thread reads `mu_device[0]` and applies
`positions_x/y/z[i] *= mu`. Because `apply` runs after the thermostat's
`apply_post` in the post-force tail (`framework.md`), the rescaled
positions follow the post-step velocity contributions. C-rescale
operates only on positions.
