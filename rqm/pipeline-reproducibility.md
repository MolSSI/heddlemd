...
test using the existing `lj_pair_force`, `reduce_pair_forces`,
...
Feature: End-to-end pipeline reproducibility
  ...
  Background:
    ...
    And two independent pipeline instances A and B, each consisting of fresh ParticleBuffers, fresh PairBuffer, and a fresh neighbor_counts CudaSlice constructed from byte-identical ParticleState inputs

  @rq-b2314952
  Scenario: Bit-exact equality after a single full velocity-Verlet step
    Given each pipeline has been warmed up with one (lj_pair_force, reduce_pair_forces) pass
    When one full step (vv_kick_drift, lj_pair_force, reduce_pair_forces, vv_kick) is executed on each pipeline
