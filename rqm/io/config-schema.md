...
One field is accepted for **every** `kind` and governs the coupling
cadence:
...
Feature: TOML simulation config schema
  ...
  Scenario: [thermostat] coupling_interval accepts a value greater than 1
    Given the Background config with [integrator] kind="velocity-verlet"
    And a [thermostat] section with kind="csvr", temperature=300.0, tau=1.0e-13,
      seed=42, coupling_interval=10
    When load_config is called
    Then it returns Ok(config)
    And the thermostat's coupling_interval equals 10

  @rq-10b0c99c
  Scenario: [thermostat] coupling_interval = 0 is rejected
    Given the Background config with [integrator] kind="velocity-verlet"
    And a [thermostat] section with kind="csvr", temperature=300.0, tau=1.0e-13,
      seed=42, coupling_interval=0
    When load_config is called
    Then it returns Err(ConfigError) reporting coupling_interval must be >= 1

  @rq-4c5a5c07
  Scenario: [thermostat] coupling_interval is accepted for every kind
    Given the Background config with [integrator] kind="velocity-verlet"
    And a [thermostat] section for each kind (nose-hoover-chain, csvr, andersen,
      berendsen) with its required fields plus coupling_interval=5
    When load_config is called for each
    Then each returns Ok(config) with coupling_interval = 5
